...
ITERATION_TIMEOUT_S = int(os.environ.get("STUDY_ITERATION_TIMEOUT_S", 300))
```
...
3. Load all 200 abstracts from `corpus/abstracts/`.
...
5. Return `CorpusRunResult(results=list[ExtractionResult], failures=list[CorpusAbstractFailure], duration_seconds=float, corpus_token_usage=CorpusTokenUsage)`.
...
the `asyncio.TimeoutError` propagates to `study_runner.py`.
...
`response_format={"type": "json_object"}`. Parses response. Validates against
`edit_protocol` schema using pydantic. On any failure: return `AgentFailure`.
...
j. Commit iteration (edits + snapshot + artifacts + ledger appends).
```
...
| 300s timeout, in-process | DECISION 002-G |
